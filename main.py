...
class Table:
    def __init__(self, canvas, width, height):
        ...
        self.stat_food = None
        self.stat_count = [None for i in range(0, 81)]
        self.stat_count_value = [0 for i in range(0, 81)]
        self.draw_stats()

    def next(self):
        self.step += 1
        if self.step % 400 >= 0 and self.step % 400 < 100:
            photo_eff = 1.2
        if self.step % 400 >= 100 and self.step % 400 < 200:
            photo_eff = 0.3
        if self.step % 400 >= 200 and self.step % 400 < 300:
            photo_eff = 0.02
        if self.step % 400 >= 300 and self.step % 400 < 400:
            photo_eff = 0.3
        for i in self.life:
            i.next()
        self.food_refresh()
...
class Alive:
    ...
    def next(self):
        ...
        self.energy -= 0.1 + (0.7 * (0.05 + self.speed)) + (0.09 * self.age)
    ...
    def death(self):
        ...
        self.table.food_data[self.x][self.y] += 15
...
def main():
    root = tkinter.Tk()
    root.title("Evolve")
    canvas = tkinter.Canvas(root, width=800, height=700, bg='white')
    canvas.pack(fill=tkinter.BOTH, expand=1)
    canvas.create_line(0, 600, 800, 600, fill=rgb(0, 0, 0))
